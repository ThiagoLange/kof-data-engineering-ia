...
Verificar que `kof 0.4.7-beta` está instalado e que todos os datasets estão acessíveis a partir da raiz do repo. Este lab deve ser o primeiro a passar; falhas aqui bloqueiam os demais.
...
- `kof version` — deve reportar `0.4.7-beta`.
- `kof info` — plataforma, JVM 25 embedded, stdlib 0.4.7.
...
- `kof debug` (DAP) e `kof profile` são JVM-only (verificados em 0.4.7).
