...
Verificar que `kof 0.1.3-beta` está instalado e que todos os datasets estão acessíveis a partir da raiz do repo. Este lab deve ser o primeiro a passar; falhas aqui bloqueiam os demais.

## Conceito

O treinamento assume `kof` no `PATH` com JVM 21+ (virtual threads) e execução a partir da raiz (`datasets/` relativo). Ferramentas verificadas:

- `kof version` — deve reportar `0.1.3-beta`.
- `kof info` — plataforma, JVM, stdlib 0.1.3.
...
- `kof debug` (DAP) e `kof profile` são JVM-only e experimentais em 0.1.3-beta.
