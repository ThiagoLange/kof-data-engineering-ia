...
- `Directory(...).createDirectories()` (funciona desde 0.4.4; `File.mkdir()` é no-op silencioso — ver `docs/LIMITACOES-KOF-0.1.3.md`) + `File.writeText` para partições.
- Watermark como `csv` append-only — auditável.

## Gaps
- Sem `kof.db` catálogo ainda; próxima iteração usa SQLite nativo.
- `File.mkdir()` não cria diretórios (no-op silencioso em 0.4.4); usar `Directory.createDirectories()`.
