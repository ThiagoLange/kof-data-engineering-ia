...
- **Client-side (KofJS):** `<canvas id="chart">` + JS `getContext("2d")` `fillRect`/`fillText` (`UI009` Canvas fechado em 0.3.2, mantido em 0.4.9; `UI003` Table/Ul/Ol já OK). Em Kof, `kof.ui.Canvas` gera o mesmo JS via `KofJS` engine.
...
- `kof.ui` Window ainda `UI003` gap no Native; `Canvas` fecha `UI009` (desde 0.3.2, mantido em 0.4.9).
