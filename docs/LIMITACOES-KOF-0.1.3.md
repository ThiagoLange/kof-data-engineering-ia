# Limitações da Linguagem Kof — Versão 0.4.10-beta

> Documento de diagnóstico baseado em experimentação contra o compilador
> instalado localmente (`~/.kof/bin/kof`, kof 0.4.10-beta, JVM backend).
> Última atualização: 2026-09-24 — contém **apenas limitações verificadas com a versão nova** (0.4.10, release só com bump de versão, sem mudança no compilador).
...
> (retorno anulável) foram movidos para “Corrigidos”. 0.4.5–0.4.10 não quebraram
> nenhum dos 51 labs (verificados `0 failed` em 0.4.10).

---

## 1. Palavras reservadas `fn`/`fun`/`func` (PARSE085) — mantida em 0.4.10
...
**Causa:** Em 0.3.2 `fn`/`fun`/`func` viraram palavras reservadas (`SG-001` `b5d3f2a`). Mantido em 0.4.10 (verificado hoje, intencional, não bug).
...
```
Verificado em 0.4.10 (`kof 0.4.10-beta`, `config.get("app.name", "default")`).
...
Verificado em 0.4.10 (`/tmp/opencode/v_pkg/mod/a.kof` + `b.kof`). `kof test <arquivo>` é isolado por arquivo — comportamento inconsistente.
...
**Sintoma 4a (verificado em 0.4.10):** `File("/tmp/x").mkdir()` compila e roda (`stmt ok`), mas o diretório **não** é criado e nenhum erro é emitido.

**Sintoma 4b (verificado em 0.4.10):** `var r = File("/tmp/x").mkdir()` + `r.toString()` (ou `mkdir().toString()` direto) quebra com:
```
...
**Mitigação:** Usar `Directory("path").createDirectories()` (verificado em 0.4.10, cria de verdade — ver `modulo-08-data/01-lakehouse/lab.kof:13` com partições `lake/city=<city>/part-v<ver>.jsonl`). Candidato a issue upstream.

---

## 0.4.5–0.4.10: verificado sem impacto no curso

| Mudança 0.4.5–0.4.10 | Status em 0.4.10 | Ação no curso |
|---|---|---|
...
| 0.4.10 = só bump de versão (`[skip ci]`, sem mudança no compilador) | ✅ 51 labs + probes re-verificados hoje, tudo igual a 0.4.9 | Nenhuma |

## Corrigidos em 0.4.4 (mantidos em 0.4.10)

| Bug | 0.3.22 | 0.4.4+ | Ação no curso |
|-----|-------|-------|---------------|
| Retorno anulável `Int?` (`VerifyError: foo()I areturn`) | ❌ | ✅ fix (reverificado em 0.4.10) | Nenhum workaround era usado; documentado como corrigido |
| `math.parseInt/parseDouble/parseIntOrDefault` nativos | ❌ | ✅ novo (0.4.0 S13a/b, reverificado em 0.4.10) | Curso mantém parsers manuais didáticos (`shared/numberparse.kof`); nativos citados como alternativa |
...
## Consequências no desenho (0.4.10)
...
4. **`exercise.kof` em `exercise/` subdir** para evitar `PKG005` duplicate em `kof run` (module resolution desde 0.3.22, mantido em 0.4.10).
5. **Partições hive-style reais** em `08-01` via `Directory.createDirectories()` (verificado em 0.4.10; `File.mkdir()` é no-op).
6. **`kof test` e `kof run` verificados** com `0.4.10` (`51` labs `0 failed`).
