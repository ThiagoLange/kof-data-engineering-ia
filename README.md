...
> 51 labs executáveis em `kof 0.3.2-beta` (`~/.kof/bin/kof`), 11 módulos (00–10), 8 datasets, 100% `kof test` pass.

**Quickstart:**
```bash
export PATH="$HOME/.kof/bin:$PATH"
kof version  # 0.3.2-beta
...
- `docs/LIMITACOES-KOF-0.1.3.md` — `0.1.3→0.3.2` migração (3 novos gaps)
- `docs/GLOSSARIO.md` — Kof×Python/Spark
- `docs/BENCHMARKS.md` / `docs/RUBRICA.md` / `docs/EXERCISES.md`

**Datasets:** `datasets/` 8 arquivos (`iris_full.csv` 150 linhas novo em 0.3.2).

**Toolchain:** `kof 0.3.2-beta` em `~/.kof` (`~/.bashrc:205`), `kof run`/`test`/`check`/`inspect`/`profile`/`build --target native|js`.
